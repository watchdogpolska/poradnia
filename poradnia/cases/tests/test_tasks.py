...
from poradnia.cases.tasks import (
    ...
    enqueue_search_articles_for_cases_task,
...
class EnqueueSearchArticlesForCasesTaskTestCase(TestCase):
    def _run(self, case_ids, pause_seconds=30, direct_search=False):
        return enqueue_search_articles_for_cases_task.apply(
            args=[case_ids],
            kwargs={"pause_seconds": pause_seconds, "direct_search": direct_search},
        ).result

    @patch("poradnia.cases.tasks.search_articles_for_case_task.apply_async")
    ...
    def test_staggers_countdown_by_pause_seconds(self, mock_apply_async):
        ...
        mock_apply_async.assert_has_calls(
            [
                call(args=[10], kwargs={"direct_search": False}, countdown=0),
                call(args=[20], kwargs={"direct_search": False}, countdown=30),
                call(args=[30], kwargs={"direct_search": False}, countdown=60),
            ]
        )
    ...
    def test_respects_custom_pause_seconds(self, mock_apply_async):
        ...
        mock_apply_async.assert_has_calls(
            [
                call(args=[10], kwargs={"direct_search": False}, countdown=0),
                call(args=[20], kwargs={"direct_search": False}, countdown=5),
            ]
        )

    @patch("poradnia.cases.tasks.search_articles_for_case_task.apply_async")
    def test_passes_direct_search_through(self, mock_apply_async):
        self._run([10], direct_search=True)
        mock_apply_async.assert_called_once_with(
            args=[10], kwargs={"direct_search": True}, countdown=0
        )
