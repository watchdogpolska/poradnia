...
class StatsCaseReactionApiTestCase(TestCase):
    ...
    def _prepare_letters(self, letter_data, case):
        ...
        for accept, created_by, status in letter_data:
            obj = LetterFactory(created_by=created_by, case=case, status=status)
    ...
    def test_many_letters(self):
        ...
        result = self.client.get(self.url).json()
        expected = [{"date": "2015-01", "reaction_time": 59}]
        self.assertEqual(result, expected)
...
class StatsCaseUnansweredApiTestCase(TestCase):
    ...
    def test_basic(self):
        ...
        result = self.client.get(self.url).json()
        expected = [{"date": "2015-01", "count": 1}, {"date": "2015-02", "count": 2}]
        self.assertEqual(result, expected)

    def test_gap_by_month(self):
        db_data = [(1, False, datetime(2015, 1, 2)), (2, False, datetime(2015, 3, 2))]
...
class StatsUserRegisteredApiTestCase(TestCase):
    ...
    def test_basic(self):
        ...
        result = self.client.get(self.url).json()
        expected = [{"date": "2016-11", "count": 3}, {"date": "2016-12", "count": 4}]
        self.assertEqual(result, expected)

    @purge_users
    def test_gap_after_init_date(self):
        db_data = [datetime(2017, 1, 2), datetime(2017, 3, 2)]
    ...
    def test_before_start_date(self):
        db_data = [datetime(2016, 1, 2)]

        user = self._prepare_users(db_data)
        self.client.force_login(user)

        result = self.client.get(self.url).json()
        expected = [{"date": "2016-11", "count": 1}]
        self.assertEqual(result, expected)
...
class GapFillerTestCase(TestCase):
    ...
    def test_single_month_gap(self):
        qs = [{"date": "2015-01", "param": 1}, {"date": "2015-03", "param": 3}]
    ...
    def test_multiple_params(self):
        qs = [{"date": "2015-01", "a": 1, "b": 1}, {"date": "2015-03", "a": 3, "b": 4}]
...
class ValueBrowseListViewTestCase(TestCase):
    ...
    def test_valid_status_code_for_detail_page(self):
        ...
        self.assertEqual(
            response.status_code, 200, "Invalid status code on '{}'".format(self.url)
        )
...
class JSONValueListViewTestCase(TestCase):
    ...
    def test_output_contains_values(self):
        ...
        self.assertTrue(
            any(entry["value"] == self.values[0].value for entry in response["values"])
        )
    ...
    def test_metric_result_is_not_null(self):
        for key, import_path in STAT_METRICS.items():
            ...
            self.assertNotEqual(
                f(),
                None,
                "Metric '{}' result of '{}' cannot be null.".format(key, import_path),
            )
