...
from django.db.models import Case, Count, F, IntegerField, Min, Prefetch, Sum, When
...
class StatsCaseCreatedRenderView(
...
class StatsCaseCreatedApiView(
    LoginRequiredMixin, SuperuserRequiredMixin, ApiListViewMixin, View
):
    raise_exception = raise_unless_unauthenticated

    def get_object_list(self):
        qs = (
            CaseModel.objects.with_month_year()
            .values("month", "year")
            .annotate(
                open=Sum(
                    Case(
        ...
        return GapFiller(result, MONTHLY, "date", DATE_FORMAT_MONTHLY).fill_gaps()
...
class StatsCaseReactionApiView(
    LoginRequiredMixin, SuperuserRequiredMixin, ApiListViewMixin, View
):
    raise_exception = raise_unless_unauthenticated
    ...
    def get_object_list(self):
        qs = (
            CaseModel.objects.filter(letter__status=LetterModel.STATUS.done)
            .filter(letter__created_by__is_staff=True)
            .annotate(
            ...
            .values("first_accepted", "created_on")
        )
        ...
        result = [
            {
                "date": reaction_date,
                "reaction_time": int(
                    sum(deltas[reaction_date])
                    / len(deltas[reaction_date])
                    / SECONDS_IN_A_DAY
                ),
            }
...
class StatsCaseUnansweredRenderView(
    LoginRequiredMixin, SuperuserRequiredMixin, TemplateView
):
...
class StatsCaseUnansweredApiView(
    LoginRequiredMixin, SuperuserRequiredMixin, ApiListViewMixin, View
):
    raise_exception = raise_unless_unauthenticated
    ...
    def get_object_list(self):
        qs = (
            CaseModel.objects.filter(last_send=None)
            .with_month_year()
            .values("month", "year")
        ...
        return GapFiller(qs, MONTHLY, "date", DATE_FORMAT_MONTHLY).fill_gaps()
...
class StatsLetterCreatedApiView(
    LoginRequiredMixin, SuperuserRequiredMixin, ApiListViewMixin, View
):
    raise_exception = raise_unless_unauthenticated
    ...
    def get_object_list(self):
        qs = (
            LetterModel.objects.with_month_year()
            .values("month", "year")
            .annotate(
                staff=Sum(
                    Case(
            ...
            .values("month", "year", "client", "staff")
            .order_by(F("year"), F("month"))
        )
...
class StatsUserRegisteredRenderView(
    LoginRequiredMixin, SuperuserRequiredMixin, TemplateView
):
...
class StatsUserRegisteredApiView(
    LoginRequiredMixin, SuperuserRequiredMixin, ApiListViewMixin, View
):
    raise_exception = raise_unless_unauthenticated
    ...
    def get_object_list(self):
        qs = (
            UserModel.objects.with_month_year()
            .values("month", "year")
            .annotate(count=Count("pk"))
            .order_by(F("year"), F("month"))
        )
        ...
        def not_after_start_date(x):
            return (
                ...
                or (x["year"] < start_date[0])
                or (x["year"] == start_date[0] and x["month"] <= start_date[1])
            )
        ...
        filled = GapFiller(serialized, MONTHLY, "date", DATE_FORMAT_MONTHLY).fill_gaps()

        total_count = 0
        ans = []
        for obj in filled:
            total_count += obj["count"]
            ans.append({"date": obj["date"], "count": total_count})
...
class ValueListView(TimeMixin):
    ...
    def item(self):
        return get_object_or_404(
            Item.objects.for_user(self.request.user), key=self.kwargs["key"]
        )

    def get_queryset(self):
        # TODO: Debug why time__date__* make fails on TravisCI
        return Value.objects.filter(
            time__lte=self.end, time__gte=self.start, item=self.item
        ).all()
...
class ValueBrowseListView(ValueListView, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["today"], kwargs["start"], kwargs["end"] = (
            self.today,
            self.start,
            self.end,
        )
...
class GraphTimeMixin(TimeMixin):
    ...
    def object(self):
        if not getattr(self, "_object", None):
            values_qs = Value.objects.filter(
                time__lte=self.end, time__gte=self.start
            ).all()
            prefetch_obj = Prefetch("items__value_set", values_qs)
            graph_qs = (
                Graph.objects.prefetch_related("items")
                .prefetch_related(prefetch_obj)
                .all()
            )
    ...
    def get_dataset(self, item, times):
        values = {
            value.time.strftime("%s"): value.value for value in item.value_set.all()
        }
    ...
    def get_graph(self):
        dataset = [
            self.get_dataset(item, self.times) for item in self.object.items.all()
        ]
    ...
    def get_table(self):
        ...
        for time in self.times:
            data_row = OrderedDict(
                (item["key"], data[item["key"]].get(time, None)) for item in header
            )
            body.append(
                {
                    "date": time,
