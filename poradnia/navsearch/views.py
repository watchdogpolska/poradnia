...
class AutocompleteView(View):
    def get_user_queryset(self, q):
        return User.objects.for_user(self.request.user).filter(
    ...
    def get_case_id_queryset(self, q):
        if q.replace('#', '').isdigit():
            return Case.objects.for_user(self.request.user).filter(id=q.replace('#', ''))[:3]
