...
class ContentSourceFactory(DjangoModelFactory):
    ...
    base_url = factory.LazyAttribute(lambda obj: f"https://{obj.name.lower().replace(' ', '')}.example.com")
    api_endpoint = factory.LazyAttribute(lambda obj: f"{obj.base_url}/wp-json/wp/v2/posts")
    is_active = True
