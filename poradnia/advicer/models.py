...
class Advice(models.Model):
    ...
    ai_assistant_tags = models.JSONField(
        verbose_name=_("AI assistant tags"),
        null=True,
        blank=True,
        help_text=_("Tags generated by AI assistant"),
    )
    ai_tags_request = models.ForeignKey(
        to="ai_assistant.N8nCaseTagsRequest",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        related_name="advices",
        verbose_name=_("AI tags request"),
        help_text=_("The AI request that produced the currently stored AI suggestion"),
    )
    ...
    def ai_institution_kind(self):
        ...
        if pk is None:
            return None
        return (
            InstitutionKind.objects.filter(pk=pk).values_list("name", flat=True).first()
        )
